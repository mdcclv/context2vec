...
class CbowContext(object):
    ...
    def __init__(self, targets, contexts, word2index, stopwords, window_size, word_counts):
        ...
        self.idf = self.count2idf(
            word_counts, word2index) if word_counts is not None else None
    ...
    def context2vec(self, sent_words, position):
        ...
        bow_inds = [self.word2index[word]
                    for word in bow if word in self.word2index and word not in self.stopwords]
        if len(bow_inds) == 0:
            print(("NOTICE: Empty bow context for: " + str(sent_words)))
            print("Trying with stopwords")
            bow_inds = [self.word2index[word]
                        for word in bow if word in self.word2index]
...
class BiLstmContext(chainer.Chain):
    ...
    def context2vec(self, sent_words, position):
        ...
        sent = [self.word2index[word]
                if word in self.word2index else Toks.UNK for word in sent_words]
    ...
    def _contexts_rep(self, sent_arr):
        ...
        l2r_sent_h = []
        # we don't need the last word in the sentence
        for i in range(l2r_sent.shape[1]-1):
            c = chainer.Variable(l2r_sent[:, i])
            e = self.l2r_embed(c)
            if self.drop_ratio > 0.0:
                h = self.l2r_1(
                    F.dropout(e, ratio=self.drop_ratio, train=self.train))
            else:
                h = self.l2r_1(e)
            l2r_sent_h.append(h)
        ...
        r2l_sent_h = []
        # we don't want the last word in the sentence
        ...
        for l2r_h, r2l_h in zip(l2r_sent_h, r2l_sent_h):
            if not self.deep:
                ...
                if self.drop_ratio > 0.0:
                    ...
                    r2l_h = self.lp_r2l(
                        F.dropout(r2l_h, ratio=self.drop_ratio, train=self.train))
                else:
        ...
        if self.deep:
            ...
            for bi_h in sent_bi_h:
                if self.drop_ratio > 0.0:
                    h1 = F.relu(
                        self.l3(F.dropout(bi_h, ratio=self.drop_ratio, train=self.train)))
                    y = self.l4(
                        F.dropout(h1, ratio=self.drop_ratio, train=self.train))
                else:
