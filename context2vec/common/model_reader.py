...
class ModelReader(object):
    ...
    def __init__(self, config_file):
        ...
        print(('Config: ', params))
        self.w, self.word2index, self.index2word, self.model = self.read_model(
            params)
    ...
    def read_bow_model(self, params):
        words_file = params['config_path'] + params['words_file']
        contexts_file = params['config_path'] + \
            params['contexts_file'] if 'contexts_file' in params else None
        window_size = int(params['window_size'])
        use_stopwords = params['stopwords']

        if 'word_counts_file' in params:
            word_counts_file = params['config_path'] + \
                params['word_counts_file']
        else:
        ...
        word_counts = self.read_word_counts(
            word_counts_file) if word_counts_file is not None else None
    ...
    def read_words(self, filename):
        with open(filename, 'r') as f:
            ...
            for i, line in enumerate(f):
                ...
                w[i] = numpy.array([float(s)
                                    for s in ss[1:]], dtype=numpy.float32)
