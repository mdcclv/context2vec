...
print(('n_vocab: %d' % (len(reader.word2index)-3)))
print(('corpus size: %d' % (reader.total_words)))

cs = [reader.trimmed_word2count[w]
      for w in range(len(reader.trimmed_word2count))]
loss_func = L.NegativeSampling(
    target_word_units, cs, NEGATIVE_SAMPLING_NUM, args.ns_power)

if args.context == 'lstm':
    model = BiLstmContext(args.deep, args.gpu, reader.word2index, context_word_units,
                          lstm_hidden_units, target_word_units, loss_func, True, args.dropout)
else:
...
for epoch in range(args.epoch):
    ...
    for sent in reader.next_batch():
        ...
        # all sents in a batch are the same length
        word_count += len(sent)*len(sent[0])
        accum_mean_loss = float(accum_loss) / \
            word_count if accum_loss > 0.0 else 0.0
    ...
    print(('accum words per epoch', word_count, 'accum_loss',
